...
export function ChatInput({
      ...
      setAttachments((prev) => {
        ...
        for (let i = 0; i < tempIds.length; i++) {
          ...
          if (data) {
            ...
            console.warn(
              `[pickFiles] No staged data for tempId=${tempId} at index ${i}`,
            );
            updated = updated.map((a) =>
              a.id === tempId
  ...
  const stageBufferFiles = useCallback(async (files: globalThis.File[]) => {
    for (const file of files) {
      ...
      try {
        console.log(
          `[stageBuffer] Reading file: ${file.name} (${file.type}, ${file.size} bytes)`,
        );
  ...
  return (
    ...
    >
      <div className="w-full">
        ...
        {/* Input Row */}
        <div
          className={`relative bg-white dark:bg-card rounded-[28px] shadow-sm border p-1.5 transition-all ${dragOver ? "border-primary ring-1 ring-primary" : "border-black/10 dark:border-white/10"}`}
        >
          {selectedTarget && (
          ...
          <div className="flex items-end gap-1.5">
            ...
            <div className="flex-1 relative">
              <Textarea
                ...
                onPaste={handlePaste}
                placeholder={
                  disabled ? t("composer.gatewayDisconnectedPlaceholder") : ""
                }
                disabled={disabled}
        ...
        <div className="mt-2.5 flex items-center justify-between gap-2 text-[11px] text-muted-foreground/60 px-4">
          ...
          {hasFailedAttachments && (
            <Button
              ...
              onClick={() => {
                setAttachments((prev) =>
                  prev.filter((att) => att.status !== "error"),
                );
                void pickFiles();
              }}
            >
