    ...
    >
      ...
      {!isUser && (
        <motion.div
          ...
          transition={getFloatingTransition(prefersReducedMotion, 2.4, 4.5)}
        >
...
function MessageBubble({
  ...
  return (
    <motion.div
      ...
      whileHover={getHoverLift(prefersReducedMotion, { y: -1, scale: 1.002 })}
    >
      {isUser ? (
...
function FileCard({ file }: { file: AttachedFileMeta }) {
  ...
  return (
    <motion.div
      className="flex max-w-[220px] items-center gap-3 rounded-xl border border-black/10 bg-black/5 px-3 py-2.5 dark:border-white/10 dark:bg-white/5"
...
function ImageThumbnail({
  ...
  return (
    <motion.div
      ...
      onClick={onPreview}
      whileHover={getHoverLift(prefersReducedMotion, { y: -2, scale: 1.02 })}
      whileTap={getTapScale(prefersReducedMotion, 0.99)}
      transition={motionTransition.gentle}
    >
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {filePath && (
            <motion.div whileTap={getTapScale(prefersReducedMotion, 0.97)}>
              <Button
                variant="ghost"
                size="icon"
...
function ToolCard({ name, input }: { name: string; input: unknown }) {
  ...
  return (
    <motion.div
      className="rounded-xl border border-black/10 bg-black/5 text-[14px] dark:border-white/10 dark:bg-white/5"
      whileHover={getHoverLift(prefersReducedMotion, { y: -1, scale: 1.002 })}
      transition={motionTransition.gentle}
    >
      <motion.button
      ...
      <AnimatePresence initial={false}>
        {expanded && input != null && (
          <motion.pre
            className="overflow-x-auto px-3 pb-2 text-xs text-muted-foreground"
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }}
            transition={motionTransition.soft}
