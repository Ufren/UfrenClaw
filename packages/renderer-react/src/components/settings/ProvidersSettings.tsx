    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground bg-black/5 dark:bg-white/5 rounded-3xl border border-transparent border-dashed">
          ...
          <h3 className="text-[15px] font-medium mb-1 text-foreground">
          ...
          <Button
            onClick={() => setShowAddDialog(true)}
            className="rounded-full px-6 h-10 bg-[#0a84ff] hover:bg-[#007aff] text-white"
          >
            <Plus className="h-4 w-4 mr-2" />
            ...
            <ProviderCard
              ...
              onSaveEdits={async (payload) => {
                ...
                if (payload.updates) {
                  ...
                  if (payload.updates.model !== undefined)
                    updates.model = payload.updates.model;
                  ...
                  if (payload.updates.fallbackProviderIds !== undefined) {
                    updates.fallbackAccountIds =
                      payload.updates.fallbackProviderIds;
                  }
                }
                await updateAccount(
                  item.account.id,
              ...
              onValidateKey={(key, options) =>
                validateAccountApiKey(item.account.id, key, options)
              }
      ...
      {showAddDialog && (
        <AddProviderDialog
          ...
          onValidateKey={(type, key, options) =>
            validateAccountApiKey(type, key, options)
          }
          devModeUnlocked={devModeUnlocked}
        />
      )}
...
function ProviderCard({
  ...
  const fallbackOptions = allProviders.filter(
  ...
  const handleSaveEdits = async () => {
    ...
    try {
...

      {
        ...
        if (
          !fallbackModelsEqual(normalizedFallbackModels, account.fallbackModels)
        ) {
        ...
        if (
          ...
          updates.fallbackProviderIds =
            normalizeFallbackProviderIds(fallbackProviderIds);
        }
      ...
      if (
        ...
        payload.newApiKey = resolveProviderApiKeyForSave(
          account.vendorId,
          "",
        ) as string;
      }
  ...
  const currentLabelClasses = isDefault
    ...
    >
          ...
          <div className="h-[42px] w-[42px] shrink-0 flex items-center justify-center text-foreground border border-black/5 dark:border-white/10 rounded-full bg-black/5 dark:bg-white/5 shadow-sm group-hover:scale-105 transition-transform">
            {getProviderIconUrl(account.vendorId) ? (
              <img
                ...
                className={cn(
                  "h-5 w-5",
                  shouldInvertInDark(account.vendorId) && "dark:invert",
                )}
              />
            ) : (
              <span className="text-xl">
                {vendor?.icon || typeInfo?.icon || "⚙️"}
              </span>
            )}
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5 text-[13px] text-muted-foreground">
              ...
              {((account.fallbackModels?.length ?? 0) > 0 ||
                (account.fallbackAccountIds?.length ?? 0) > 0) && (
                <>
                  <span className="w-1 h-1 rounded-full bg-black/20 dark:bg-white/20" />
                  <span
                    className="truncate max-w-[150px]"
                    title={t("aiProviders.sections.fallback")}
                  >
                      ...
                      ...normalizeFallbackProviderIds(
                        ...
                        .map(
                          (fallbackId) =>
                            allProviders.find(
                              (candidate) =>
                                candidate.account.id === fallbackId,
                            )?.account.label,
                        )
                        .filter(Boolean),
                    ].join(", ")}
      ...
      {isEditing && (
        <div className="space-y-6 mt-4 pt-4 border-t border-black/5 dark:border-white/5">
          ...
          {canEditModelConfig && (
            <div className="space-y-3">
              <p className={currentSectionLabelClasses}>
              ...
              {showModelIdField && (
                <div className="space-y-1.5 pt-2">
                  ...
                  <Input
                    ...
                    placeholder={
                      typeInfo?.modelIdPlaceholder || "provider/model-id"
                    }
                    className={currentInputClasses}
                  />
                </div>
              ...
              <div className="space-y-3 pt-2">
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    placeholder={t(
                      "aiProviders.dialog.fallbackModelIdsPlaceholder",
                    )}
                    className={
                      isDefault
                    ...
                    >
                      {fallbackOptions.map((candidate) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={fallbackProviderIds.includes(
                              candidate.account.id,
                            )}
                            onChange={() =>
                              toggleFallbackProvider(candidate.account.id)
                            }
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isDefault
                      ...
                      : "h-[44px] bg-black/5 dark:bg-muted hover:bg-black/5 dark:hover:bg-white/10 shadow-sm",
                  )}
                  disabled={
                    validating ||
                    saving ||
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isDefault
                      ...
                      : "h-[44px] w-[44px] bg-black/5 dark:bg-muted border border-black/10 dark:border-white/10 hover:bg-black/10 dark:hover:bg-white/10 shadow-sm text-muted-foreground hover:text-foreground",
                  )}
                >
                  <X className="h-4 w-4" />
                </Button>
...
function AddProviderDialog({
  ...
  const preferredOAuthMode = selectedVendor?.supportedAuthModes.includes(
    "oauth_browser",
  )
    ? "oauth_browser"
  ...
  useEffect(() => {
    ...
    setAuthMode(
      selectedVendor.defaultAuthMode === "api_key" ? "apikey" : "oauth",
    );
  ...
  const pendingOAuthRef = React.useRef<{
    accountId: string;
    label: string;
  } | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleSuccess = async (data: unknown) => {
      ...
      const accountId =
        payload?.accountId || pendingOAuthRef.current?.accountId;
  ...
  const handleStartOAuth = async () => {
    ...
    if (
      selectedType === "minimax-portal" &&
      existingVendorIds.has("minimax-portal-cn")
    ) {
      toast.error(t("aiProviders.toast.minimaxConflict"));
      return;
    }
    if (
      selectedType === "minimax-portal-cn" &&
      existingVendorIds.has("minimax-portal")
    ) {
      toast.error(t("aiProviders.toast.minimaxConflict"));
      return;
    }
        ...
        apiKey.trim(),
        {
          ...
          apiProtocol:
            selectedType === "custom" || selectedType === "ollama"
              ? apiProtocol
              : undefined,
          model: resolveProviderModelForSave(
            typeInfo,
            modelId,
            devModeUnlocked,
          ),
                  ...
                  <div className="h-12 w-12 mx-auto mb-3 flex items-center justify-center bg-black/5 dark:bg-white/5 rounded-xl shadow-sm border border-black/5 dark:border-white/5 group-hover:scale-105 transition-transform">
                    {getProviderIconUrl(type.id) ? (
                      <img
                        ...
                        className={cn(
                          "h-6 w-6",
                          shouldInvertInDark(type.id) && "dark:invert",
                        )}
                      />
                    ) : (
            ...
            <div className="space-y-6">
                ...
                <div className="h-10 w-10 shrink-0 flex items-center justify-center bg-black/5 dark:bg-white/5 rounded-xl">
                  {getProviderIconUrl(selectedType!) ? (
                    <img
                      ...
                      className={cn(
                        "h-6 w-6",
                        shouldInvertInDark(selectedType!) && "dark:invert",
                      )}
                    />
                  ) : (
              ...
              <div className="space-y-6 bg-transparent p-0">
                ...
                {isOAuth && supportsApiKey && (
                  <div className="flex rounded-xl border border-black/10 dark:border-white/10 overflow-hidden text-[13px] font-medium shadow-sm bg-black/5 dark:bg-muted p-1 gap-1">
                    <button
                        ...
                        <div className="relative z-10 flex flex-col items-center justify-center text-center space-y-5">
                          {oauthError ? (
                            ...
                            <div className="space-y-5 w-full">
                              ...
                              <div className="flex items-center justify-center gap-3 p-4 bg-black/5 dark:bg-muted border border-black/5 dark:border-white/5 rounded-xl shadow-inner">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    toast.success(
                                      t("aiProviders.oauth.codeCopied"),
                                    );
                                  }}
                                >
              ...
              <div className="flex justify-end gap-3">
                <Button
                  onClick={handleAdd}
                  className={cn(
                    "rounded-full px-8 h-[42px] text-[13px] font-semibold bg-[#0a84ff] hover:bg-[#007aff] text-white shadow-sm",
                    useOAuthFlow && "hidden",
                  )}
                  disabled={
                    !selectedType ||
