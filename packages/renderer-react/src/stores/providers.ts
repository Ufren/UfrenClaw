...
export const useProviderStore = create<ProviderState>((set, get) => ({
  ...
  setDefaultAccount: async (accountId) => {
    try {
      ...
      if (!result.success) {
        throw new Error(
          result.error || "Failed to set default provider account",
        );
      }
  ...
  validateAccountApiKey: async (providerId, apiKey, options) => {
    try {
      const result = await hostApiFetch<{ valid: boolean; error?: string }>(
  ...
  validateApiKey: async (providerId, apiKey, options) =>
    get().validateAccountApiKey(providerId, apiKey, options),
