...
import {
  browserOAuthManager,
  type BrowserOAuthProviderType,
} from "../utils/browser-oauth";
  ...
  ipcMain.handle("hostapi:fetch", async (_, request: HostApiFetchRequest) => {
    try {
      ...
      const response = await proxyAwareFetch(
        `http://127.0.0.1:${PORTS.UFRENCLAW_HOST_API}${path}`,
        {
          method,
          headers,
          body,
...
function registerUnifiedRequestHandlers(gatewayManager: GatewayManager): void {
  ...
  ipcMain.handle(
    "app:request",
    async (_, request: AppRequest): Promise<AppResponse> => {
      if (
        !request ||
        ...
        switch (request.module) {
          ...
          case "provider": {
            ...
            if (request.action === "get") {
              const payload = request.payload as
                ...
                | undefined;
              const providerId =
                typeof payload === "string" ? payload : payload?.providerId;
            ...
            if (request.action === "hasApiKey") {
              ...
              const providerId =
                typeof payload === "string" ? payload : payload?.providerId;
              if (!providerId)
                throw new Error("Invalid provider.hasApiKey payload");
            ...
            if (request.action === "getApiKey") {
              ...
              const providerId =
                typeof payload === "string" ? payload : payload?.providerId;
              if (!providerId)
                throw new Error("Invalid provider.getApiKey payload");
            ...
            if (request.action === "validateKey") {
              const payload = request.payload as
                ...
                | undefined;
              const providerId = Array.isArray(payload)
                ? payload[0]
                : payload?.providerId;
              const apiKey = Array.isArray(payload)
                ? payload[1]
                : payload?.apiKey;
            ...
            if (request.action === "save") {
              ...
              try {
                ...
                if (apiKey !== undefined) {
                  ...
                  if (trimmedKey) {
                    await providerService.setLegacyProviderApiKey(
                      config.id,
                      trimmedKey,
                    );
                  }
            ...
            if (request.action === "delete") {
              ...
              const providerId =
                typeof payload === "string" ? payload : payload?.providerId;
              if (!providerId)
                throw new Error("Invalid provider.delete payload");
            ...
            if (request.action === "updateWithKey") {
              ...
              if (!providerId || !updates)
                throw new Error("Invalid provider.updateWithKey payload");
              ...
              try {
                ...
                try {
                  ...
                  console.warn(
                    "Failed to rollback provider updateWithKey:",
                    rollbackError,
                  );
                }
            ...
            if (request.action === "setDefault") {
              ...
              const providerId =
                typeof payload === "string" ? payload : payload?.providerId;
              if (!providerId)
                throw new Error("Invalid provider.setDefault payload");
              ...
              try {
                ...
                if (provider) {
                  try {
                    await syncDefaultProviderToRuntime(
                      providerId,
                      gatewayManager,
                    );
                  } catch (err) {
          ...
          case "update": {
            ...
            if (request.action === "setChannel") {
              const payload = request.payload as
                ...
                | undefined;
              const channel =
                typeof payload === "string" ? payload : payload?.channel;
              if (!channel)
                throw new Error("Invalid update.setChannel payload");
            ...
            if (request.action === "setAutoDownload") {
              const payload = request.payload as
                ...
                | undefined;
              const enable =
                typeof payload === "boolean" ? payload : payload?.enable;
          ...
          case "cron": {
            ...
            if (request.action === "create") {
              ...
              const created = await gatewayManager.rpc(
                "cron.add",
                gatewayInput,
              );
              data =
            ...
            if (request.action === "toggle") {
              const payload = request.payload as
                ...
                | undefined;
              const id = Array.isArray(payload) ? payload[0] : payload?.id;
              const enabled = Array.isArray(payload)
                ? payload[1]
                : payload?.enabled;
              if (!id || typeof enabled !== "boolean")
                throw new Error("Invalid cron.toggle payload");
              data = await gatewayManager.rpc("cron.update", {
                id,
                patch: { enabled },
              });
              break;
            ...
            if (request.action === "trigger") {
              ...
              data = await gatewayManager.rpc("cron.run", {
                id,
                mode: "force",
              });
              break;
          ...
          case "usage": {
            if (request.action === "recentTokenHistory") {
              const payload = request.payload as
                ...
                | undefined;
              const limit =
                typeof payload === "number" ? payload : payload?.limit;
              const safeLimit =
...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the UfrenClaw chat page.
    ...
    async (
      ...
      try {
        const gatewayInput = {
          ...
          wakeMode: "next-heartbeat",
          sessionTarget: "isolated",
          // UI-created jobs deliver results via UfrenClaw WebSocket chat events,
  ...
  ipcMain.handle(
    "cron:update",
    async (_, id: string, input: Record<string, unknown>) => {
      try {
  ...
  ipcMain.handle("cron:toggle", async (_, id: string, enabled: boolean) => {
    try {
      const result = await gatewayManager.rpc("cron.update", {
        id,
        patch: { enabled },
      });
      return result;
  ...
  ipcMain.handle("cron:trigger", async (_, id: string) => {
    try {
      const result = await gatewayManager.rpc("cron.run", {
        id,
        mode: "force",
      });
      return result;
...
function registerGatewayHandlers(
  ...
  ipcMain.handle(
    "gateway:rpc",
    async (_, method: string, params?: unknown, timeoutMs?: number) => {
      try {
  ...
  ipcMain.handle(
    "gateway:httpProxy",
    async (_, request: GatewayHttpProxyRequest) => {
      try {
        const status = gatewayManager.getStatus();
        const port = status.port || 18789;
        const path =
          request?.path && request.path.startsWith("/") ? request.path : "/";
        const method = (request?.method || "GET").toUpperCase();
        const timeoutMs =
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        logger.info(
          `[chat:sendWithMedia] RPC result: ${JSON.stringify(result)}`,
        );
        return { success: true, result };
      } catch (error) {
...
function registerOpenClawHandlers(gatewayManager: GatewayManager): void {
  ...
  async function ensureDingTalkPluginInstalled(): Promise<{
    ...
    if (!sourceDir) {
      logger.warn(
        "Bundled DingTalk plugin mirror not found in candidate paths",
        { candidateSources },
      );
      return {
        installed: false,
    ...
    try {
      mkdirSync(join(homedir(), ".openclaw", "extensions"), {
        recursive: true,
      });
      ...
      if (!existsSync(targetManifest)) {
        return {
          installed: false,
          warning:
            "Failed to install DingTalk plugin mirror (manifest missing).",
        };
      }

      logger.info(
        `Installed DingTalk plugin from bundled mirror: ${sourceDir}`,
      );
      return { installed: true };
    } catch (error) {
      logger.warn(
        "Failed to install DingTalk plugin from bundled mirror:",
        error,
      );
      return {
        installed: false,
  ...
  async function ensureWeComPluginInstalled(): Promise<{
    ...
    if (!sourceDir) {
      logger.warn("Bundled WeCom plugin mirror not found in candidate paths", {
        candidateSources,
      });
      return {
        installed: false,
  ...
  async function ensureQQBotPluginInstalled(): Promise<{
    ...
    try {
      ...
      logger.warn(
        "Failed to install QQ Bot plugin from bundled mirror:",
        error,
      );
      return {
        installed: false,
  ...
  ipcMain.handle(
    ...
    async (_, channelType: string, config: Record<string, unknown>) => {
      try {
        logger.info("channel:saveConfig", {
          channelType,
        ...
        if (channelType === "qqbot") {
          ...
          if (gatewayManager.getStatus().state !== "stopped") {
            logger.info(
              `Scheduling Gateway reload after channel:saveConfig (${channelType})`,
            );
  ...
  ipcMain.handle(
    "channel:setEnabled",
    async (_, channelType: string, enabled: boolean) => {
      try {
        await setChannelEnabled(channelType, enabled);
        scheduleGatewayChannelRestart(
          `channel:setEnabled (${channelType}, enabled=${enabled})`,
        );
        return { success: true };
      } catch (error) {
...
function registerProviderHandlers(gatewayManager: GatewayManager): void {
  ...
  ipcMain.handle(
    "provider:save",
    async (_, config: ProviderConfig, apiKey?: string) => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(
          `[UfrenClaw-validate] validating provider type: ${providerType}`,
        );
...
function registerAppHandlers(): void {
  ...
  // Set language
  ipcMain.handle("app:setLanguage", async (_, language: string) => {
      ...
      try {
        ...
        const result = await dialog.showSaveDialog({
          ...
          filters: [
            {
              name: "Images",
              extensions: [ext, "png", "jpg", "jpeg", "webp", "gif"],
            },
  ...
  ipcMain.handle("session:delete", async (_, sessionKey: string) => {
    try {
      ...
      if (parts.length < 3) {
        return {
          success: false,
          error: `sessionKey has too few parts: ${sessionKey}`,
        };
      }
      ...
      try {
        ...
        logger.warn(
          `[session:delete] Could not read sessions.json: ${String(e)}`,
        );
        return {
          success: false,
          error: `Could not read sessions.json: ${String(e)}`,
        };
      }
      ...
      if (Array.isArray(sessionsJson.sessions)) {
        const entry = (
          sessionsJson.sessions as Array<Record<string, unknown>>
        ).find((s) => s.key === sessionKey || s.sessionKey === sessionKey);
        if (entry) {
          // Could be "file", "fileName", "id" + ".jsonl", or "path"
          uuidFileName = (entry.file ?? entry.fileName ?? entry.path) as
            | string
            | undefined;
      ...
      if (!uuidFileName && sessionsJson[sessionKey] != null) {
        ...
        if (typeof val === "string") {
          ...
          const absFile = (entry.sessionFile ??
            entry.file ??
            entry.fileName ??
          ...
          if (absFile) {
            ...
            if (uuidVal)
              uuidFileName = uuidVal.endsWith(".jsonl")
                ? uuidVal
                : `${uuidVal}.jsonl`;
          }
        }
      ...
      if (!uuidFileName && !resolvedSrcPath) {
        ...
        return {
          success: false,
          error: `Cannot resolve file for session: ${sessionKey}`,
        };
      }
      ...
      try {
        ...
        if (Array.isArray(json2.sessions)) {
          json2.sessions = (
            json2.sessions as Array<Record<string, unknown>>
        ...
        logger.info(
          `[session:delete] Removed "${sessionKey}" from sessions.json`,
        );
      } catch (e) {
        logger.warn(
          `[session:delete] Could not update sessions.json: ${String(e)}`,
        );
