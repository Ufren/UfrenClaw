...
const LINUX_AUTOSTART_FILE = join(".config", "autostart", "UfrenClaw.desktop");
...
function getLinuxDesktopEntry(): string {
  return [
    ...
    "Version=1.0",
    "Name=UfrenClaw",
    "Comment=UfrenClaw - AI Assistant",
...
async function applyLinuxLaunchAtStartup(enabled: boolean): Promise<void> {
  ...
  logger.info(
    `Launch-at-startup disabled and desktop entry removed: ${targetPath}`,
  );
}
...
function applyWindowsOrMacLaunchAtStartup(enabled: boolean): void {
  ...
  logger.info(
    `Launch-at-startup ${enabled ? "enabled" : "disabled"} via login items`,
  );
}

export async function applyLaunchAtStartupSetting(
  enabled: boolean,
): Promise<void> {
  try {
    ...
    logger.warn(
      `Launch-at-startup unsupported on platform: ${process.platform}`,
    );
  } catch (error) {
