...
import {
  ensureUfrenClawContext,
  repairUfrenClawOnlyBootstrapFiles,
...
// On Wayland this maps the running window to ufren.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === "linux") {
  // Electron 的 app 模块没有 setDesktopName，Linux 下用环境变量 CHROME_DESKTOP 指定 .desktop 文件
  process.env.CHROME_DESKTOP = "ufren.desktop";
}
...
function createWindow(): BrowserWindow {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(join(__dirname, "../../renderer-react/dist/index.html"));
  }
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info("=== UfrenClaw Application Starting ===");
  logger.debug(
  ...
  // Sync tray menu language with persisted settings
  const savedLanguage = await getSetting("language");
  updateTrayMenu(savedLanguage);
  ...
  // Repair any bootstrap files that only contain UfrenClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureUfrenClawContext()
  // previously created the file before the gateway could seed the full template.
  void repairUfrenClawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on("status", (status: { state: string }) => {
    ...
    if (status.state === "running") {
      void ensureUfrenClawContext().catch((error) => {
        logger.warn(
          "Failed to re-merge UfrenClaw context after gateway reconnect:",
          error,
        );
      });
    }
  ...
  // Merge UfrenClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureUfrenClawContext will retry until the target files appear.
  void ensureUfrenClawContext().catch((error) => {
    logger.warn("Failed to merge UfrenClaw context into workspace:", error);
  });
