...
import { ensureUfrenClawContext, repairUfrenClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running') {
      void ensureUfrenClawContext().catch((error) => {
        logger.warn('Failed to re-merge UfrenClaw context after gateway reconnect:', error);
      });
    }
  });
