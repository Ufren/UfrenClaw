...
export class ClawHubService {
  ...
  private async runCommand(args: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
      if (this.useNodeRunner && !fs.existsSync(this.cliEntryPath)) {
        reject(
          new Error(`ClawHub CLI entry not found at: ${this.cliEntryPath}`),
        );
        return;
      }
      ...
      const spawnArgs = useShell
        ? commandArgs.map((a) => quoteForCmd(a))
        : commandArgs;
      ...
      return lines
        .map((line) => {
          ...
          const match = cleanLine.match(
            /^(\S+)\s+v?(\d+\.\S+)\s+(.+? ago|just now|yesterday)\s+(.+)$/i,
          );
          if (match) {
            return {
              slug: match[1],
  ...
  async openSkillReadme(
    ...
    const skillDir =
      directSkillDir || this.resolveSkillDirByManifestName(uniqueCandidates);
