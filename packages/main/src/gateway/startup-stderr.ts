...
export function classifyGatewayStderrMessage(
  message: string,
): GatewayStderrClassification {
  ...
  if (msg.includes("DeprecationWarning"))
    return { level: "debug", normalized: msg };
  if (msg.includes("Debugger attached"))
    return { level: "debug", normalized: msg };
...
export function recordGatewayStartupStderrLine(
  lines: string[],
  line: string,
): void {
