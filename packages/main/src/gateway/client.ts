...
export class GatewayClient {
  ...
  async getSkillConfig(skillId: string): Promise<Record<string, unknown>> {
    return this.manager.rpc<Record<string, unknown>>("skills.getConfig", {
      skillId,
    });
  }
  ...
  async testProvider(
    ...
    return this.manager.rpc<{ success: boolean; error?: string }>(
      "providers.test",
      { providerId },
    );
  }
