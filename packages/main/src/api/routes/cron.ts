...
export async function handleCronRoutes(
  ...
  if (url.pathname === "/api/cron/jobs" && req.method === "GET") {
    try {
      const result = await ctx.gatewayManager.rpc("cron.list", {
    ...
    try {
      ...
      sendJson(
        ...
        result && typeof result === "object"
          ? transformCronJob(result as GatewayCronJob)
          : result,
      );
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
  ...
  if (url.pathname.startsWith("/api/cron/jobs/") && req.method === "PUT") {
    try {
      const id = decodeURIComponent(
        url.pathname.slice("/api/cron/jobs/".length),
      );
      ...
      sendJson(
        res,
        200,
        await ctx.gatewayManager.rpc("cron.update", { id, patch }),
      );
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === "/api/cron/trigger" && req.method === "POST") {
    try {
      ...
      sendJson(
        res,
        200,
        await ctx.gatewayManager.rpc("cron.run", {
          id: body.id,
          mode: "force",
        }),
      );
