...
const CHANNEL_TOP_LEVEL_KEYS_TO_KEEP = new Set([
  "accounts",
  "defaultAccount",
  "enabled",
]);
...
export function toOpenClawChannelType(channelType: string): string {
  return channelType === UI_WECHAT_CHANNEL_TYPE
    ? OPENCLAW_WECHAT_CHANNEL_TYPE
    : channelType;
}

export function toUiChannelType(channelType: string): string {
  return channelType === OPENCLAW_WECHAT_CHANNEL_TYPE
...
function ensurePluginAllowlist(
  ...
  if (channelType === OPENCLAW_WECHAT_CHANNEL_TYPE) {
    ...
    if (!normalizedAllow.includes(OPENCLAW_WECHAT_CHANNEL_TYPE)) {
  ...
  if (channelType === "feishu") {
    if (!currentConfig.plugins) {
      ...
      const normalizedAllow = allow.filter(
        (pluginId) =>
          pluginId !== "feishu" && pluginId !== LEGACY_FEISHU_PLUGIN_ID,
      );
      ...
      // Remove legacy entries that can conflict with the current plugin ID.
...
function transformChannelConfig(
  ...
  if (channelType === "feishu" || channelType === "wecom") {
    const existingDmPolicy =
      existingAccountConfig.dmPolicy === "pairing"
        ? "open"
        : existingAccountConfig.dmPolicy;
    transformedConfig.dmPolicy =
      transformedConfig.dmPolicy ?? existingDmPolicy ?? "open";

    let allowFrom = (transformedConfig.allowFrom ??
      existingAccountConfig.allowFrom ?? ["*"]) as string[];
...
function getLegacyChannelPayload(
...
function migrateLegacyChannelConfigToAccounts(
  ...
  const hasAccounts =
    ...
    Object.keys(channelSection.accounts as Record<string, ChannelConfigData>)
      .length > 0;
  ...
  accounts[defaultAccountId] = {
    ...(channelSection.enabled !== undefined
      ? { enabled: channelSection.enabled }
      : {}),
    ...legacyPayload,
  ...
  channelSection.defaultAccount =
    typeof channelSection.defaultAccount === "string" &&
...
export async function saveChannelConfig(
  ...
  const resolvedChannelType = toOpenClawChannelType(channelType);
  const resolvedAccountId = normalizeOpenClawAccountId(accountId);
  ...
  const existingAccountConfig = resolveAccountConfig(
    channelSection,
    resolvedAccountId,
  );
  ...
  console.log(
    `Saved channel config for ${resolvedChannelType} account ${resolvedAccountId}`,
  );
}
  ...
  if (channelType === "discord") {
    ...
    if (guilds) {
      ...
      if (guildIds.length > 0) {
        ...
        const channels = guildConfig?.channels as
...
export async function getChannelFormValues(
  ...
  const values = extractFormValues(toOpenClawChannelType(channelType), saved);
  return Object.keys(values).length > 0 ? values : undefined;
}

export async function deleteChannelAccountConfig(
  channelType: string,
  accountId: string,
): Promise<void> {
  const currentConfig = await readOpenClawConfig();
  ...
  const accounts = channelSection.accounts as
    | Record<string, ChannelConfigData>
    | undefined;
  if (!accounts?.[resolvedAccountId]) return;
  ...
  await writeOpenClawConfig(currentConfig);
  if (resolvedChannelType === OPENCLAW_WECHAT_CHANNEL_TYPE) {
    await deleteWeChatAccountState(resolvedAccountId).catch(() => {});
  }
  logger.info("Deleted channel account config", {
    channelType: resolvedChannelType,
    accountId: resolvedAccountId,
  });
  console.log(
    `Deleted channel account config for ${resolvedChannelType}/${resolvedAccountId}`,
  );
}
...
export async function deleteChannelConfig(channelType: string): Promise<void> {
  ...
  if (currentConfig.channels?.[resolvedChannelType]) {
    ...
    if (currentConfig.plugins?.entries?.[resolvedChannelType]) {
      ...
      if (
        currentConfig.plugins &&
        Object.keys(currentConfig.plugins).length === 0
      ) {
  ...
  if (resolvedChannelType === OPENCLAW_WECHAT_CHANNEL_TYPE) {
    await clearWeChatLoginState().catch(() => {});
  }
}
...
export async function listConfiguredChannels(): Promise<string[]> {
  ...
  try {
    ...
    if (await fileExists(whatsappDir)) {
      ...
      if (hasSession) channels.add("whatsapp");
    }
  } catch {
...
export async function deleteAgentChannelAccounts(
  agentId: string,
): Promise<void> {
  const currentConfig = await readOpenClawConfig();
  if (!currentConfig.channels) return;

  const accountId = normalizeOpenClawAccountId(
    agentId === "main" ? DEFAULT_ACCOUNT_ID : agentId,
  );
  let modified = false;
  ...
  for (const channelType of Object.keys(currentConfig.channels)) {
    ...
    if (channelType === OPENCLAW_WECHAT_CHANNEL_TYPE) {
      await deleteWeChatAccountState(accountId).catch(() => {});
    }
    if (Object.keys(accounts).length === 0) {
  ...
  if (modified) {
    await writeOpenClawConfig(currentConfig);
    logger.info("Deleted all channel accounts for agent", {
      agentId,
      accountId,
    });
  }
...
export async function setChannelEnabled(
  ...
  if (PLUGIN_CHANNELS.includes(resolvedChannelType)) {
    ...
    await writeOpenClawConfig(currentConfig);
    console.log(
      `Set plugin channel ${resolvedChannelType} enabled: ${enabled}`,
    );
    return;
  }
  ...
  try {
    ...
    if (!meResponse.ok) {
      if (meResponse.status === 401) {
        return {
          valid: false,
          errors: ["Invalid bot token. Please check and try again."],
          warnings: [],
        };
      }
      ...
      const msg =
        ...
        `Discord API error: ${meResponse.status}`;
    ...
    if (!meData.bot) {
      return {
        valid: false,
        errors: [
          "The provided token belongs to a user account, not a bot. Please use a bot token.",
        ],
        warnings: [],
      };
    }
    ...
    return {
      valid: false,
      errors: [
        `Connection error when validating bot token: ${error instanceof Error ? error.message : String(error)}`,
      ],
      warnings: [],
    };
  }
    ...
    try {
      ...
      if (!guildResponse.ok) {
        if (guildResponse.status === 403 || guildResponse.status === 404) {
          ...
          result.errors.push(
            `Failed to verify guild ID: Discord API returned ${guildResponse.status}`,
          );
          result.valid = false;
        }
      } else {
      ...
      result.warnings.push(
        `Could not verify guild ID: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
    ...
    try {
      ...
      if (!channelResponse.ok) {
        if (channelResponse.status === 403 || channelResponse.status === 404) {
          ...
          result.errors.push(
            `Failed to verify channel ID: Discord API returned ${channelResponse.status}`,
          );
          result.valid = false;
        }
      } else {
      ...
      result.warnings.push(
        `Could not verify channel ID: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
...
async function validateTelegramCredentials(
  ...
  if (!allowedUsers)
    return {
      valid: false,
      errors: ["At least one allowed user ID is required"],
      warnings: [],
    };
...
export async function validateChannelConfig(
  ...
  const resolvedChannelType = toOpenClawChannelType(channelType);
  const uiChannelType = toUiChannelType(resolvedChannelType);
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const matchesChannel =
        lowerLine.includes(resolvedChannelType) ||
        lowerLine.includes(uiChannelType);
      if (matchesChannel && lowerLine.includes("error")) {
        result.errors.push(line.trim());
        result.valid = false;
      } else if (matchesChannel && lowerLine.includes("warning")) {
        result.warnings.push(line.trim());
      } else if (lowerLine.includes("unrecognized key") && matchesChannel) {
        result.errors.push(line.trim());
        result.valid = false;
      }
    }
    ...
    const config = await readOpenClawConfig();
    const savedChannelConfig = await getChannelConfig(
      resolvedChannelType,
      DEFAULT_ACCOUNT_ID,
    );
    if (!config.channels?.[resolvedChannelType] || !savedChannelConfig) {
      result.errors.push(`Channel ${uiChannelType} is not configured`);
      result.valid = false;
    } else if (config.channels[resolvedChannelType].enabled === false) {
      result.warnings.push(`Channel ${uiChannelType} is disabled`);
    }
    ...
    if (
      errorMessage.includes("Unrecognized key") ||
      ...
      result.errors.push(
        "OpenClaw not found. Please ensure OpenClaw is installed.",
      );
      result.valid = false;
    } else {
