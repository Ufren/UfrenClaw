...
/**
 * Merge a UfrenClaw context section into an existing file's content.
...
/**
 * Detect and remove bootstrap .md files that contain only UfrenClaw markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairUfrenClawOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(UfrenClaw_BEGIN);
      const endIdx = content.indexOf(UfrenClaw_END);
...
/**
 * Merge UfrenClaw context snippets into workspace bootstrap files that
...
/**
 * Ensure UfrenClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureUfrenClawContext(): Promise<void> {
  let skipped = await mergeUfrenClawContextOnce();
  if (skipped === 0) return;
