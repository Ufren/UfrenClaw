...
function resolveUvBin(): {
  ...
  if (app.isPackaged) {
    ...
    logger.warn(
      `Bundled uv binary not found at ${bundled}, falling back to system PATH`,
    );
  }
...
export async function installUv(): Promise<void> {
  ...
  if (!isAvailable) {
    ...
    throw new Error(
      `uv not found in system PATH and bundled binary missing at ${bin}`,
    );
  }
  ...
  try {
    ...
    if (hasMirror) {
      ...
      try {
        ...
        logger.error(
          "Python install attempt 2 (no mirror) also failed:",
          secondError,
        );
        throw secondError;
      }
