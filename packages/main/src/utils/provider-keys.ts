...
const OPENCLAW_OAUTH_PLUGIN_PROVIDER_KEY_SET = new Set<string>(
  OPENCLAW_OAUTH_PLUGIN_PROVIDER_KEYS,
);
...
export function getOAuthProviderTargetKey(type: string): string | undefined {
  ...
  return isMiniMaxProviderType(type)
    ? OPENCLAW_PROVIDER_KEY_MINIMAX
    : OPENCLAW_PROVIDER_KEY_QWEN;
}
...
export function getOAuthProviderDefaultBaseUrl(
  type: string,
): string | undefined {
  if (!isOAuthProviderType(type)) return undefined;
  if (type === OPENCLAW_PROVIDER_KEY_MINIMAX)
    return "https://api.minimax.io/anthropic";
  if (type === "minimax-portal-cn") return "https://api.minimaxi.com/anthropic";
  return "https://portal.qwen.ai/v1";
}

export function normalizeOAuthBaseUrl(
  type: string,
  baseUrl?: string,
): string | undefined {
