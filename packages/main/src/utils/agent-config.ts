...
function normalizeAgentsConfig(config: AgentConfigDocument): {
  ...
  const rawEntries = Array.isArray(agentsConfig.list)
    ? agentsConfig.list.filter(
        (entry): entry is AgentListEntry =>
          Boolean(entry) &&
          typeof entry === "object" &&
  ...
  const defaultEntry =
    rawEntries.find((entry) => entry.default) ?? rawEntries[0];
  return {
    agentsConfig,
...
function isChannelBinding(binding: unknown): binding is BindingConfig {
  ...
  if (
    ...
    return false;
  if (typeof candidate.match.channel !== "string" || !candidate.match.channel)
    return false;
...
function getManagedWorkspaceDirectory(agent: AgentListEntry): string | null {
  ...
  const configuredWorkspace = expandPath(
  ...
  const normalizedConfigured = trimTrailingSeparators(
    normalize(configuredWorkspace),
  );
  const normalizedManaged = trimTrailingSeparators(normalize(managedWorkspace));

  return normalizedConfigured === normalizedManaged
    ? configuredWorkspace
    : null;
}

async function removeAgentWorkspaceDirectory(
...
async function provisionAgentFilesystem(
  ...
  const mainEntry =
    entries.find((entry) => entry.id === MAIN_AGENT_ID) ??
    createImplicitMainEntry(config);
  const sourceWorkspace = expandPath(
    mainEntry.workspace || getDefaultWorkspacePath(config),
  );
  const targetWorkspace = expandPath(
    agent.workspace || `~/.openclaw/workspace-${agent.id}`,
  );
  const sourceAgentDir = expandPath(
    mainEntry.agentDir || getDefaultAgentDirPath(MAIN_AGENT_ID),
  );
  const targetAgentDir = expandPath(
    agent.agentDir || getDefaultAgentDirPath(agent.id),
  );
  const targetSessionsDir = join(
    getOpenClawConfigDir(),
    "agents",
    agent.id,
    "sessions",
...
function listConfiguredAccountIdsForChannel(
  config: AgentConfigDocument,
  channelType: string,
): string[] {
...
async function buildSnapshotFromConfig(
  ...
  const { channelToAgent, accountToAgent } = getChannelBindingMap(
    config.bindings,
  );
  ...
  const agents: AgentSummary[] = entries.map((entry) => {
    const modelLabel =
      formatModelLabel(entry.model) || defaultModelLabel || "Not configured";
    const inheritedModel =
      !formatModelLabel(entry.model) && Boolean(defaultModelLabel);
    const entryIdNorm = normalizeAgentIdForBinding(entry.id);
    const ownedChannels =
      agentChannelSets.get(entryIdNorm) ?? new Set<string>();
    return {
      id: entry.id,
      name:
        entry.name || (entry.id === MAIN_AGENT_ID ? MAIN_AGENT_NAME : entry.id),
      isDefault: entry.id === defaultAgentId,
      modelDisplay: modelLabel,
      inheritedModel,
      workspace:
        entry.workspace ||
        (entry.id === MAIN_AGENT_ID
          ? getDefaultWorkspacePath(config)
          : `~/.openclaw/workspace-${entry.id}`),
...
export async function createAgent(name: string): Promise<AgentsSnapshot> {
  const config = (await readOpenClawConfig()) as AgentConfigDocument;
  const { agentsConfig, entries, syntheticMain } =
  ...
  const nextEntries = syntheticMain
    ? [
        createImplicitMainEntry(config),
        ...entries.filter((_, index) => index > 0),
      ]
    : [...entries];
  ...
  if (
    !nextEntries.some((entry) => entry.id === MAIN_AGENT_ID) &&
    syntheticMain
  ) {
...
export async function deleteAgentConfig(
  ...
  const config = (await readOpenClawConfig()) as AgentConfigDocument;
  const { agentsConfig, entries, defaultAgentId } =
    normalizeAgentsConfig(config);
  ...
  config.bindings = Array.isArray(config.bindings)
    ? config.bindings.filter(
        (binding) =>
          !(isChannelBinding(binding) && binding.agentId === agentId),
      )
    : undefined;
...
export async function clearAllBindingsForChannel(
  channelType: string,
): Promise<void> {
  const config = (await readOpenClawConfig()) as AgentConfigDocument;
