...
async function setSkillsEnabled(
  skillKeys: string[],
  enabled: boolean,
): Promise<void> {
 ...
 */
export async function getAllSkillConfigs(): Promise<
...
/**
 * Built-in skills bundled with UfrenClaw that should be pre-deployed to
...
const PREINSTALLED_MARKER_NAME = ".UfrenClaw-preinstalled.json";
...
async function readPreinstalledManifest(): Promise<PreinstalledSkillSpec[]> {
  ...
  try {
    ...
    return parsed.skills.filter((s): s is PreinstalledSkillSpec =>
      Boolean(s?.slug),
    );
  } catch (error) {
...
async function readPreinstalledLockVersions(
  sourceRoot: string,
...
async function tryReadMarker(
  markerPath: string,
): Promise<PreinstalledMarker | null> {
...
export async function ensurePreinstalledSkillsInstalled(): Promise<void> {
  ...
  if (!sourceRoot) {
    logger.warn(
      "Preinstalled skills source root not found; skipping preinstall.",
    );
    return;
  }
  ...
  for (const spec of skills) {
    ...
    if (!existsSync(sourceManifest)) {
      logger.warn(
        `Preinstalled skill source missing SKILL.md, skipping: ${sourceDir}`,
      );
      continue;
    }
