...
class BrowserOAuthManager extends EventEmitter {
        ...
        provider === "google"
          ? await loginGeminiCliOAuth({
              ...
              note: async (message, title) => {
                logger.info(
                  `[BrowserOAuth] ${title || "OAuth note"}: ${message}`,
                );
              },
              prompt: async () => {
                throw new Error(
                  "Manual browser OAuth fallback is not implemented in UfrenClaw yet.",
                );
              },
              progress: {
  ...
  private async onSuccess(
    ...
    this.pendingManualCodeReject = null;
    logger.info(
      `[BrowserOAuth] Successfully completed OAuth for ${providerType}`,
    );
    ...
    const oauthTokenEmail =
      "email" in token && typeof token.email === "string"
        ? token.email
        : undefined;
