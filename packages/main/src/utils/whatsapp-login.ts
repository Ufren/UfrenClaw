...
const openclawRequire = createRequire(
  join(openclawResolvedPath, "package.json"),
);
...
function resolveOpenClawPackageJson(packageName: string): string {
  ...
  try {
    ...
    // Fallback: try resolving from the current module context (frontend dependencies)
    try {
      ...
      const fallbackReason =
        fallbackErr instanceof Error
          ...
          : String(fallbackErr);
      throw new Error(
        `Failed to resolve "${packageName}" from OpenClaw context or local fallback. ` +
          `openclawPath=${openclawPath}, resolvedPath=${openclawResolvedPath}. ` +
          `Original error: ${reason}. Fallback error: ${fallbackReason}`,
        { cause: err },
      );
    }
  }
...
const baileysPath = dirname(
  resolveOpenClawPackageJson("@whiskeysockets/baileys"),
);
const qrcodeTerminalPath = dirname(
  resolveOpenClawPackageJson("qrcode-terminal"),
);
...
const QRCodeModule = require(
  join(qrcodeTerminalPath, "vendor", "QRCode", "index.js"),
);
const QRErrorCorrectLevelModule = require(
...
export class WhatsAppLoginManager extends EventEmitter {
  ...
  private async finishLogin(accountId: string): Promise<void> {
    ...
    console.log(
      "[WhatsAppLogin] Finishing login, closing socket to hand over to Gateway...",
    );
    await this.stop();
  ...
  private async connectToWhatsApp(accountId: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[WhatsAppLogin] Could not load pino from baileys, trying root",
          e,
        );
        try {
          pino = require("pino");
          ...
          console.warn(
            "[WhatsAppLogin] Pino not found, using console fallback",
          );
      ...
      console.log(
        `[WhatsAppLogin] Starting login for ${accountId}, version: ${version}`,
      );
      ...
      this.socket = makeWASocket({
        ...
        // mobile: false,
        // browser: ['UfrenClaw', 'Chrome', '1.0.0'],
      });
      ...
      this.socket.ev.on(
        ...
        async (update: ConnectionState) => {
          try {
              ...
              if (shouldReconnect && this.active) {
                ...
                if (error?.output?.statusCode === DisconnectReason.loggedOut) {
                  try {
                    ...
                    console.error(
                      "[WhatsAppLogin] Failed to clear auth dir:",
                      err,
                    );
                  }
                }
            ...
            console.error(
              "[WhatsAppLogin] Error in connection update:",
              innerErr,
            );
          }
        },
